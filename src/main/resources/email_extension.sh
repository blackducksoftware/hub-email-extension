...
checkIsRunning() {
  if [ -f $DIR/$PID_FILE ]; then
    local running_pid=$(ps -eo pid,args | grep java | grep hub-email-extension | awk '{print $1}')
...
startExtension() {
  ...
  sleep 1s
  local pid=$(ps -eo pid,args | grep java | grep hub-email-extension | awk '{print $1}')
